...
public class StrToDcPurchaseOrder implements PurchaseOrder {

    @Override
    public BigInteger createOrder() {
        int poNbr = new Random().nextInt(1000);
        System.out.println("Created an Order for Store To DC with PO Nbr: "+poNbr);
        return new BigInteger(""+poNbr);
    }

    @Override
    public BigDecimal auditOrder(BigInteger poNbr) {
        System.out.println("Audit for an PO Nbr: "+poNbr+" which is for Store To DC for amount: 100$");
